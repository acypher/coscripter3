...
| Task | Notes |
|---|---|
| **6a. Data model** | Named tables in `chrome.storage.local`; columns + rows; cell = string or link URL |
| **6b. Scratch editor UI** | New **Tables** tab: list tables, open editor (HTML `<table>` or simple grid) |
| **6c. Cell references in parser** | Extend `parseTarget()` for `cell in the "col" column of row N of the "name" scratchtable` |
